...
local function refreshBuffs()
	for frame in pairs(triggers) do
		...
		for i, button in ipairs(frame.debuffs) do
			local auraData = C_UnitAuras.GetAuraDataByIndex(frame.unit, i, debuffFilter)
			if (auraData ~= nil and auraData.name) then
				button.Icon:SetTexture(auraData.icon)
				button.Count:SetText(auraData.count and auraData.count > 1 or "")
				local color = DebuffTypeColor[auraData.dispelType or "none"]
		...
		for i, button in ipairs(frame.buffs) do
			...
			local auraData = C_UnitAuras.GetAuraDataByIndex(frame.unit, i, buffFilter)
			if (auraData ~= nil and auraData.name) then
				button.Icon:SetTexture(auraData.icon)
				button:Show()
			else
